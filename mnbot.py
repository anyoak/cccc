...
def show_number_management(chat_id):
    try:
        ...
        markup.add(add_btn, view_btn, delete_btn)
        markup.row(back_btn)
...
def delete_country(chat_id, country_code):
    try:
        ...
        bot.send_message(chat_id, f"✅ {country['flag']} {country['name']} and all its numbers have been deleted.")
    except Exception as e:
...
def parse_csv_file(content):
    ...
    try:
        ...
        for row in reader:
            for item in row:
                num = extract_number_from_text(str(item))
                if num:
                    numbers.append(num)
    except:
        text = content.decode('utf-8', errors='ignore')
        matches = re.findall(r'\+\d{10,15}|\b\d{10,15}\b', text)
        numbers = list(set(matches))
